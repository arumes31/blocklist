...
func (p *PostgresRepository) BulkCreatePersistentBlocks(ips []string, entries []models.IPEntry) error {
	...
	return conn.Raw(func(driverConn any) error {
		...
		for i, ip := range ips {
			...
			batch.Queue("INSERT INTO persistent_blocks (ip, timestamp, reason, added_by, geo_json) VALUES ($1, $2, $3, $4, $5) ON CONFLICT (ip) DO UPDATE SET timestamp = $2, reason = $3, added_by = $4, geo_json = $5",
				ip, entries[i].Timestamp, entries[i].Reason, entries[i].AddedBy, geoJSON)
		}
